...
def sobol_indicies_from_emulator(df_in: pd.DataFrame, params_in: list, bounds_in: list, quantity_mean_in: str,
    ...
    y = gpe.train_and_predict(df_in, params_in, quantity_mean_in, X)

    # Input thereduced problem dictionary, the one with constants removed
    Si = sobol.analyze(problem_reduced, y,print_to_console=True)

    first_Si=pd.DataFrame(Si['S1'])
    total_Si=pd.DataFrame(Si['ST'])
    first_Si.columns=["S1"]
    total_Si.columns = ["ST"]
